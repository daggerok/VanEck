...
- **All ETFs** (`All`) plus one tab per VanEck asset class (category).
...
| Key | Content |
| --- | --- |
| `vaneck-selected-etfs` | JSON array of selected ETF tickers |
| `vaneck-active-fund` | active fund ticker (removed when none) |
| `vaneck-blacklisted-etfs` | JSON array of blacklisted tickers |
| `vaneck-tab-sorts` | JSON map `tab -> { key, dir }` of **explicitly chosen** sorts |
| `vaneck-tab-filters` | JSON map `tab -> query` of **explicitly entered** filters |
| `vaneck-site-state` | auxiliary state: `sheetFilter` mirror of the per-tab filter map plus the last `activeTab` |
| `vaneck-theme` | `dark` / `light` |
| `vaneck-searches` | **legacy** pre-rename filter key; migrated at boot and removed on first write |
...
- Every tab remembers its own search query in `vaneck-tab-filters`.
...
- `vaneck-site-state.sheetFilter` mirrors the dedicated filter map so
...
- `vaneck-selected-etfs` / `vaneck-active-fund` in localStorage.
...
- The Watchlist table mirrors this: `Loading holdings of N selected ETFs…`
  ...
  not available yet. Run bun ./scripts/update-data.ts to refresh the VanEck feed.`
...
The VanEck feed publishes one `Identifier` column per holding row plus
`Ticker` and `Name`. That column is the **Bloomberg FIGI** — VanEck's daily
holdings download literally heads it `Identifier (FIGI)`, and the updater
normalizes it to `Identifier`. VanEck publishes **no CUSIP, ISIN or SEDOL**,
so the `C:`/`I:`/`S:` tiers of the fallback chain below are reachable only
through the EDGAR N-PORT-P fallback, never from a VanEck download.

The fixed-income sheet has **no Ticker column at all**, so for those rows the
FIGI is the only identifier and the `D:` tier does the work. The same issuer
appears once per maturity, which is precisely why the dedupe must not fall
back to the name while a FIGI exists.
...
**Placeholder rule:** all-zero identifiers (`000000000`, published by some
EDGAR N-PORT filings) are treated as missing too, so those rows fall through
to the published name instead of collapsing into a single garbage key
(`D:000000000` must never appear in the aggregation).

VanEck's own sheets use `--` for "not published", and the updater's
`cleanCell()` collapses `--`, `-`, `—`, `–`, `N/A`, `NA`, `NONE` and `NULL`
to an empty string before the row reaches the feed. So a VanEck row whose
Ticker **and** Identifier are both `--` (the `Other/Cash` balancing row every
equity fund carries) resolves on tier 6, by name — and because all three
funds with holdings publish that same row, it aggregates into one entry with
`# ETFs = 3`.
...
cash positions, futures/swaps/derivatives and zero-weight rows with otherwise
valid data all remain in the aggregation.

`scripts/ui.test.ts` (test 12) proves this against the real feed, using the
cases VanEck actually publishes:

- `-USD CASH-` balances in GDX, SMH and OIH — a currency label in the Ticker
  column, no FIGI at all — are kept under `T:-USD CASH-`, and SMH's
  **negative** `-0.02%` balance survives the aggregation (the summed weight is
  `0.28%`, not the `0.30%` you would get by dropping it);
- the `--` / `Other/Cash` row keys on `N:OTHER/CASH` and aggregates across all
  three funds;
- non-US listings keep their local exchange suffix as a distinct key
  (`T:NST AU`, `T:2259 HK`);
- a holding with no FIGI (`XPRO` in OIH) still keys on its ticker.
...
- `api/vaneck/index.json` — catalog; `funds[].holdings`/`funds[].history`
  agree with the per-fund manifests (validated by the acceptance tests' feed
  oracle, test 15).
- `api/vaneck/funds/<TICKER>/meta.json` — `holdings.pages[]` and
  `history.pages[]` manifests plus identifiers, yields and inline
  Distribution rows.
- Page rows are **keyed objects** whose key set is exactly the `headers`
  array published in the same envelope. `writeFundPages()` asserts that
  invariant for every row before writing, and the acceptance tests re-check it
  across the whole feed.
  - Holdings, equity sheet: `Name`, `Ticker`, `Identifier`, `Shares Held`,
    `Asset Category`, `Market Value`, `Notional Value`, `Weight`.
  - Holdings, fixed-income sheet: `Name`, `Maturity`, `Identifier`, `Coupon`,
    `Asset Category`, `Par Value`, `Market Value`, `Weight`, `Country`,
    `Currency` — note the absence of `Ticker`.
  - History: `Date`, `NAV`, `Close`, `Volume`, `Premium/Discount`,
    `Total Net Assets`, `Index Level`. VanEck publishes no adjusted-close
    series, so there is no `Adj Close` column.
- The `Identifier` column holds the **FIGI**; `meta.json.identifiers` records
  `cusip: null`, `isin: null` and notes that the FIGI is published per
  holding, not per fund.
- The CSV/TXT export uses a fixed 7-column projection — `Name`, `Ticker`,
  `Identifier`, `Weight`, `Market Value`, `Shares Held`, `Asset Category` —
  which is a subset of the stored page headers, so both sheet shapes export
  without change.
- Distributions would render the actual ex-date under the `Ex-Date` column.
  VanEck publishes no per-fund distributions download, so `distributions.rows`
  is empty for every fund and the tab shows its explanatory empty state.
...
file-backed `fetch` over `api/vaneck/`) and covers:
...
12. identifier fallbacks: placeholder tickers fall through to the published
    name, cash and negative-weight rows are kept, non-US local tickers stay
    distinct, and numeric local tickers (`005930`) are valid keys;
