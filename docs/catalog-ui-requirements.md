...
| 10 | `SEC Yield` | `secYield` | official Investment Finder value first, else the fund page header, where server-rendered | `—` for funds that don't publish it |
| 11 | `Frequency` | `dividendFrequency` | official Investment Finder declared schedule, else derived (see below) | coded for sorting |
...
`Dist Yield` and `12M Yield` (added, then removed) duplicated `Dividend
Yield` in 70 of 72 populated funds — not a distinct metric worth a column of
its own. The underlying `distributionYield`/`yield12M` fields still exist in
`meta.json.metrics` and feed the Overview tab's Distributions section; they
just aren't columns in the main table.
