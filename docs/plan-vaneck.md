...
| `/investments/etf-<ticker>/` is a greedy 302 (`etf-einc` → `dynamic-high-income-etf-inc` → 404) | A static slug table `scripts/vaneck-slugs.ts` (88 entries, harvested 2026-09-19 from `/etf-mutual-fund-finder/etfs/?InvType=etf&tab=ov` chunks 0-7/11) is authoritative; `vaneckFundPageUrl()` uses it, `etf-<ticker>` is only a fallback for unknown tickers |
...
| The ETF Guide lists 88 ETFs, the finder 91 | The delta is logged; no rows are invented |
