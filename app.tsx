/**
 * @file VanEck Watchlist Application
 * Client-side static feed viewer for api/vaneck/** with multi-ETF Watchlist
...
const INDEX_URL = './api/vaneck/index.json';
const THEME_KEY = 'vaneck-theme';
...
const LEGACY_FILTERS_KEY = 'vaneck-searches'; // pre-rename key, migrated at boot
...
const COLUMN_TOOLTIPS: Record<string, string> = {
  ...
  Ticker: 'Ticker Symbol — Unique stock market identifier. For holdings: the Ticker column exactly as published in the VanEck daily holdings download, which carries the local exchange suffix for non-US listings (NST AU, 2259 HK, BRMS IJ) and a label such as -USD CASH- for cash balances. "—" when the position has no exchange ticker — VanEck\'s fixed-income sheet has no Ticker column at all, so those rows key on the Identifier.',
  'Fund Name': 'Fund Name — Official legal name of the VanEck exchange-traded fund (ETF), as published on the VanEck fund page.',
  Category: 'Category — vaneck.com groups its ETFs by asset class ("Equity", "Income", "Real Assets | Commodities"); the tab shows that provider asset-class label verbatim, and the fund page breadcrumb grouping is kept in meta.json.',
  Name: 'Security Name — Full registered legal name of the company or underlying financial asset.',
  Identifier: 'Identifier (FIGI) — Bloomberg FIGI exactly as published in the VanEck daily holdings download (the column is literally headed "Identifier (FIGI)"). VanEck publishes no ISIN and does not server-render a CUSIP. Positions without an exchange ticker (bonds, cash, futures) are identified in the Watchlist by this.',
  SEDOL: 'SEDOL — Stock Exchange Daily Official List identifier. Not published by VanEck; shown as "—" (data limitation).',
  FIGI: 'FIGI — Bloomberg Financial Instrument Global Identifier, the security identifier VanEck publishes in its daily holdings download.',
  TER: 'Expense Ratio — Total annual fund operating expenses as a % of assets, as published in the VanEck fund page header. VanEck prints Gross and Net separately where they differ; the Net figure is shown.',
  NAV: 'NAV (Net Asset Value) — Per-share dollar value of the fund, as published in the VanEck fund page header.',
  'Net Assets': 'Net Assets (AUM) — Total Net Assets as published in the VanEck fund page header.',
  Weight: 'Weight — Position weight as a percentage of the fund\'s total net assets, from the "% of Net Assets" column of the daily holdings download.',
  ...
  'Dividend Yield': 'Dividend Yield — VanEck does not publish a yield in the server-rendered fund page header, so where a value appears it is the indicated yield: latest distribution per share x payments per year / NAV, derived from the Yahoo dividend history and labelled as indicated. "—" when no distribution history is available.',
  'SEC Yield': 'SEC Yield (30-Day) — Not available from VanEck for this feed: vaneck.com serves its 30-day SEC yield through a client-side widget that the static updater cannot read, and neither the daily holdings download nor the NAV / premium-discount history carries it. Shown as "—" (data limitation).',
  ...
  'TR 1Y': 'TR 1Y (1-Year Total Return) — Not published in the server-rendered VanEck fund page: the Performance panel is loaded by client-side script, so the static updater cannot read it. Shown as "—" (data limitation).',
  'TR 3Y': 'TR 3Y (3-Year Total Return) — Cumulative return over 3 years. Not readable from the server-rendered VanEck fund page (client-side Performance panel); shown as "—" (data limitation).',
  'TR 5Y': 'TR 5Y (5-Year Total Return) — Cumulative return over 5 years. Not readable from the server-rendered VanEck fund page (client-side Performance panel); shown as "—" (data limitation).',
  'TR 10Y': 'TR 10Y (10-Year Total Return) — Cumulative return over 10 years. Not readable from the server-rendered VanEck fund page (client-side Performance panel); shown as "—" (data limitation).',
  'CAGR 3Y': 'CAGR 3Y (3-Year Compound Annual Growth Rate) — Not readable from the server-rendered VanEck fund page (client-side Performance panel); shown as "—" (data limitation).',
  'CAGR 5Y': 'CAGR 5Y (5-Year Compound Annual Growth Rate) — Not readable from the server-rendered VanEck fund page (client-side Performance panel); shown as "—" (data limitation).',
  'CAGR 10Y': 'CAGR 10Y (10-Year Compound Annual Growth Rate) — Not readable from the server-rendered VanEck fund page (client-side Performance panel); shown as "—" (data limitation).',
  YTD: 'Year-to-date return published in the VanEck fund page header.',
  '1Y': '1-year total return. Not readable from the server-rendered VanEck fund page (client-side Performance panel); shown as "—" (data limitation).',
  '3Y': '3-year average annual total return. Not readable from the server-rendered VanEck fund page; shown as "—" (data limitation).',
  '5Y': '5-year average annual total return. Not readable from the server-rendered VanEck fund page; shown as "—" (data limitation).',
  '10Y': '10-year average annual total return. Not readable from the server-rendered VanEck fund page; shown as "—" (data limitation).',
  'SI Ann.': 'Since-inception annualized total return. Not readable from the server-rendered VanEck fund page; shown as "—" (data limitation).',
  'Return As Of': 'As-of date of the return figures published in the VanEck fund page header.',
  Inception: 'Inception Date — fund launch date as published in the VanEck fund page header.',
  Exchange: 'Primary listing exchange.',
  Close: 'Last Trade — most recent closing market price, from the VanEck daily NAV history.',
  'Prem/Disc': 'Premium / Discount — closing price versus NAV (%), from the VanEck daily NAV history.',
  ...
  Frequency: 'Distribution frequency — coded for sorting from fund.distributions.frequency (derived in scripts/update-data.ts by inferDistributionFrequency from the Yahoo Finance dividend-history feed). Codes: 00 — / None / Unknown, 01 Monthly, 04 Quarterly, 06 Semi-annually, 12 Annually, 99 Irregular. The Overview tab shows the raw label.',
  'Ex-Date': 'Ex-dividend date of the latest distribution. VanEck publishes no distributions download, so this column is empty for this feed.',
  Dividend: 'Latest dividend per share. VanEck publishes no distributions download, so this column is empty for this feed.',
  Coupon: 'Bond annual coupon rate (%), from the fixed-income holdings sheet.',
  Maturity: 'Bond maturity date, from the fixed-income holdings sheet.',
  'Market Value': 'Position market value, as published in the daily holdings download.',
...
function exportFileName(scope: string, extension: string): string {
  ...
  return `vaneck-${scope.toLowerCase().replace(/\s+/g, '-')}-${stamp}.${extension}`;
}
...
// =========================================================================
// 4. Static API loading & paginated sheets (api/vaneck/**, iShares-style)
// =========================================================================
...
async function loadCatalog(): Promise<void> {
  setStatus('Loading VanEck ETF data from api/vaneck/index.json…', 'info');
...
async function loadFundMeta(ticker: string): Promise<any> {
  ...
  const request = (async () => {
    try {
      const meta = await fetchJson(`./api/vaneck/funds/${encodeURIComponent(ticker)}/meta.json`);
...
async function fetchPage(ticker: string, pagePath: string): Promise<{ headers: string[]; rows: string[][] }> {
  ...
  const page = await fetchJson(`./api/vaneck/funds/${encodeURIComponent(ticker)}/${path}`);
  ...
  if (!state.selected.size) {
    ...
    el.tableBody.innerHTML = `<tr><td colspan="8" class="py-12 text-center text-slate-400 dark:text-slate-500">${
      loading
        ...
        : 'Holdings data is not available yet. Run bun ./scripts/update-data.ts to refresh the VanEck feed.'
...
function renderDistributionsTable(fund: FundRow): void {
  ...
  el.tickerCount.textContent = fund.ticker;
  renderSubtitle(`${fund.ticker} distributions · vaneck.com publishes no per-fund distributions download, so these rows come from the Yahoo dividend history (ex-date, amount) and the frequency is inferred from the cadence; funds with no available history show an explanatory state instead of a blank table.`);
}
...
function renderSubtitle(text?: string): void {
  ...
  const countsText = state.counts
    ...
    : '';
  const base = text ? String(text) : 'Search VanEck ETFs, select rows, then use the Watchlist tab.';
  ...
  el.subtitle.innerHTML = `
    ...
    <span class="block sm:inline">·${generated ? ` updated ${escapeHtml(generated)}` : ''}${countsText ? ` · ${escapeHtml(countsText)}.` : '.'} Data: <a href="./api/vaneck/index.json" target="_blank" rel="noopener noreferrer" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">api/vaneck/index.json</a> generated from <a href="https://www.vaneck.com/us/en/etf-mutual-fund-finder/" target="_blank" rel="noopener noreferrer" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">official VanEck fund pages and daily holdings downloads</a> + SEC EDGAR N-PORT-P (VanEck ETF Trust, CIK 0001137360 — holdings fallback only) + Yahoo Finance (distribution history only)</span>
  `;
...
function init(): void {
  ...
  void loadCatalog().catch(error => {
    ...
    setStatusRow(`Unable to load api/vaneck/index.json: ${message}. Run bun ./scripts/update-data.ts and serve the folder (for example bunx serve . -p 1234).`, 'error');
  });
}
