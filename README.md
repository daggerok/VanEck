...
VanEck ETF holdings to Watchlist. A single-file client-side tool reading the generated `./api/vaneck` static feed (VanEck ETF finder, per-fund product pages and daily holdings, Yahoo Finance history, SEC EDGAR N-PORT-P as fallback) into a searchable ETF/asset-class catalog with per-fund tabs, watchlist aggregation, ticker copy and CSV/TXT export — the same look, feel, columns and business logic as the sibling applications.
...
| Block | Source |
| --- | --- |
| Catalog (all US VanEck ETFs) | `https://www.vaneck.com/us/en/etf-mutual-fund-finder/` (the JSON behind the [ETF finder](https://www.vaneck.com/us/en/etf-mutual-fund-finder/)) |
| Holdings per fund | `https://www.vaneck.com/us/en/investments/{SLUG}/holdings/` (the page behind each fund, e.g. [GDX](https://www.vaneck.com/us/en/investments/gold-miners-etf-gdx/holdings/)) |
| Daily history, distributions | Yahoo Finance public chart API (`/v8/finance/chart/{TICKER}?range=max&interval=1d&events=div`) |
| Fallback | SEC EDGAR N-PORT-P: `company_tickers_mf.json` maps ticker to CIK, `browse-edgar` returns newest filing |
...
| --- | --: | --- |
| `MAX_FETCHES` | all | Batch size: with a positive value the updater continues after the committed cursor in `api/vaneck/update-state.json`; empty or `0` is a full pass — every fund is refreshed in one run. |
...
| `TICKERS` | all | Space-, comma- or semicolon-separated ticker allowlist, e.g. `GDX SMH MOAT ESPO BJK`. |
...
TICKERS="GDX SMH MOAT ESPO BJK" ./scripts/update-data.ts
