/**
 * Headless harness for the VanEck UI (index.html + app.tsx).
 *
 ...
 * generated feed from api/vaneck/. That lets bun test exercise the exact
...
function createFeedFetch(stats: FetchStats) {
  return async function fetchStub(input: string): Promise<any> {
    ...
    if (!relative.startsWith("api/vaneck/")) return notFound();
...
export async function createApp(options: { storage?: MemoryStorage } = {}): Promise<AppHandle> {
  ...
  const run = <T = any>(code: string): T => vm.runInContext(code, ctx) as T;
 ...
 * feed) also count as missing (contract §5, VanEck extension).
 */
